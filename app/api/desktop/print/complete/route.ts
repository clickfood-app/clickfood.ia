...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data, error } = await supabase.rpc("complete_order_print_job", {
      ...
      return NextResponse.json(
        {
          ...
          error: isUnauthorized
            ? "Dispositivo não autorizado para concluir impressão."
            : "Erro ao concluir impressão.",
        },
    ...
    return NextResponse.json(
      {
        ...
        error:
          error instanceof Error
            ? error.message
            : "Erro inesperado ao concluir impressão.",
      },
      { status: 500 },
    )
  }
