...
const EFI_WEBHOOK_URL =
  process.env.EFI_PRODUCTION_WEBHOOK_URL?.trim() ||
  "https://efi-webhook.clickfoodbr.com"
...
async function getAuthenticatedRestaurant() {
  ...
  if (restaurantError) {
    console.error("Erro ao buscar restaurante em /api/efi/account/webhook:", {
...
async function downloadCertificateToTempFile(certificateStoragePath: string) {
  ...
  const tempFilePath = path.join(
    os.tmpdir(),
    `efi-webhook-${crypto.randomUUID()}.p12`
  )
...
function createEfiClient(integration: EfiIntegrationRow, certificatePath: string) {
  ...
  return new EfiPay({
    sandbox: integration.environment !== "production",
...
async function saveWebhookResult({
  integrationId,
  success,
  message,
}: {
...
export async function POST() {
  ...
  try {
    ...
    if (!integration) {
      return jsonError("Configure a conta Efi antes de cadastrar o webhook.", 400)
    }

    if (!integration.pix_key) {
      return jsonError("Informe a chave Pix da Efi antes de cadastrar o webhook.", 400)
    }

    if (!integration.certificate_storage_path) {
      return jsonError("Envie o certificado .p12 da Efi antes de cadastrar o webhook.", 400)
    }
    ...
    if (typeof efipay.pixConfigWebhook !== "function") {
      throw new Error("Metodo pixConfigWebhook nao encontrado no SDK da Efi.")
    }

    const efiResponse = await efipay.pixConfigWebhook(
      {
        chave: integration.pix_key,
      },
      {
        webhookUrl: EFI_WEBHOOK_URL,
      }
    )

    await saveWebhookResult({
    ...
    return NextResponse.json({
      success: true,
      message: "Webhook Pix Efi cadastrado com sucesso.",
      restaurant: {
        id: restaurant.id,
        name: restaurant.name,
      },
      environment: integration.environment,
      webhook_url_configured: EFI_WEBHOOK_URL,
      callback_expected_at: `${EFI_WEBHOOK_URL.replace(/\/+$/, "")}/pix`,
    ...
    const message =
      error instanceof Error
        ? error.message
        : "Nao foi possivel cadastrar o webhook Efi."
    ...
    try {
      ...
      // Ignora erro secundario ao salvar resultado.
    }

    console.error("Erro em POST /api/efi/account/webhook:", normalizedError)
